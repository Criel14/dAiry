...
- `electron/main/`：主进程模块目录，按 `配置 / IPC / 窗口 / 日记 / 工作区词库 / 大模型` 等职责拆分
...
- “自动整理”按钮，支持基于当前正文草稿一次生成 `summary + tags`
- 日记信息展示开关
- `天气 / 地点 / 标签` 候选词库维护
- “新一天开始时间”设置，支持 `0` 到 `6` 点
- 设置页已包含外观、编辑器、大模型、词库、工作区五个分组
- 应用配置中已落地大模型非敏感设置，密钥配置已独立落地并通过 `safeStorage` 加密保存
- 手动保存、快捷保存与未保存内容切换提醒
...
- 工作区配置仍按这套方案设计
- 当前代码实际已落地的是应用配置 `config.json` 与密钥配置 `secrets.json`
- 工作区配置中的 `workspace.json` 仍会在后续阶段继续补齐
...
- 推荐最小字段为：
  ...
  - `model`
  - `timeoutMs`
- `providerType` 用于标识当前选择的模型提供方，例如 `openai`、`deepseek`、`alibaba`、`openai-compatible`
- `baseURL` 用于兼容官方服务、自部署网关或 OpenAI-compatible 服务
- `model` 为默认模型名
- `timeoutMs` 为主进程调用 AI 的超时时间；当前版本使用内置固定默认值，不向用户暴露编辑入口
...
- 当前代码使用 `providerType + encryptedApiKey` 的最小结构保存“当前 provider 对应的密钥”
- 后续如果需要支持多 provider 并存，再评估是否演进为按 provider 分组的结构
...
3. 读取当前工作区的词库数据，例如 `.dairy/tags.json`
4. 调用 AI 前，从 `secrets.json` 中读取并解密 `apiKey`
5. `workspace.json` 后续正式落地后，再补入运行时读取流程
...
- 第一阶段优先支持少量稳定 provider，例如 `openai`、`deepseek`、`alibaba`、`openai-compatible`
...
- AI 请求只能由主进程发起
- 结果应先给用户预览，再由用户确认写入
- 当前“自动整理”只回填到前端表单草稿，仍需用户点击“保存信息”后才会落盘到 Markdown frontmatter
...
- 当前 `JournalMetadataPanel.vue` 已支持“自动整理”，会基于当前正文草稿生成 `summary + tags`，但不会自动写盘
...
- 当前设置页已经落地 `字数热力图`、`新一天开始时间`、`日记信息展示`、`大模型配置`、`候选词库`、`工作区状态`
- 当前大模型能力已按 `electron/main/ai-config.ts`、`electron/main/ai-secrets.ts`、`electron/main/ai/` 分层组织
- 当前提示词放在 `electron/main/ai/prompts/` 目录，并通过 `prompt-loader.ts` 统一加载
